import { Stream, fetchUserStream } from "../services/twitchapi";
import { getStreamerIds, updateStreamerStatus } from "../services/streamer";
...
async function updateStreamer(channelID: string): Promise<void> {
  ...
  if (streamData.stream && streamData.stream.channel.game === "Dota 2") {
    console.log(
      grey(
        `🔴 ${channelID} | ${streamData.stream.channel.name} is live. [🎮${streamData.stream.game} | 👤${streamData.stream.viewers} | 📃${streamData.stream.channel.status}]`
      )
    );
    await updateStreamerStatus(
      channelID,
      true,
      streamData.stream.channel.status,
      streamData.stream.viewers,
      streamData.stream.preview.medium
    );
  } else if (streamData.stream) {
    console.log(
      grey(
        `🔵 ${channelID} | ${streamData.stream.channel.name} is live but is playing 🎮${streamData.stream.game} - marked as offline.`
      )
    );
