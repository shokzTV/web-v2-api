...
export async function getAllEvents(): Promise<DecoratedEvent[]> {
    ...
    const [events] = await conn.execute<EventRow[]>(`
      SELECT 
        ...
        slug
      FROM event`);
    const [eventTags] = await conn.execute<TagResponse[]>(`SELECT et.event_id as event, t.id, t.name, t.image as image, t.image_webp as imageWEBP, t.image_jpeg_2000 as imageJP2 FROM event_tags et INNER JOIN tag t ON t.id = et.tag_id`);
